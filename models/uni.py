...
Description:
    This script implements a multimodal deep learning framework for the classification of head and neck lymphoid lesions using histopathological image analysis, nuclear morphometric 
    features, and clinicopathologic data.

    The pipeline integrates feature embeddings extracted from histopathological image patches using a convolutional neural network backbone together with clinicopathologic and morphometric 
    variables processed through a fully connected multilayer perceptron (MLP). The combined multimodal representation is subsequently used for classification.
    ...
    The pipeline follows patient-level dataset separation to prevent
    data leakage and was designed for multicentre studies involving
    heterogeneous histopathological datasets.
...
# ATTENTION HEATMAPS
# =========================
def is_tissue_patch(patch_pil, white_threshold=220, tissue_fraction_threshold=0.20):
...
def create_attention_heatmap(slide_path, uni_model, mil_model, patch_model, class_names, transform, cfg, save_prefix="heatmap"):
    slide_name = os.path.splitext(os.path.basename(slide_path))[0]
    slide = openslide.OpenSlide(slide_path)
    w, h = slide.dimensions

    xs = list(range(0, max(1, w - cfg.PATCH_SIZE + 1), cfg.HEATMAP_STRIDE))
    ys = list(range(0, max(1, h - cfg.PATCH_SIZE + 1), cfg.HEATMAP_STRIDE))
    coords = [(x, y) for y in ys for x in xs]

    if cfg.MAX_HEATMAP_PATCHES is not None:
        coords = coords[:cfg.MAX_HEATMAP_PATCHES]

    embeddings = []
    valid_coords = []
    patch_scores = []

    loop = tqdm(coords, desc=f"Heatmap patches [{slide_name}]", dynamic_ncols=True, bar_format=cfg.TQDM_BAR)

    with torch.no_grad():
        for x, y in loop:
            patch = slide.read_region((x, y), 0, (cfg.PATCH_SIZE, cfg.PATCH_SIZE)).convert("RGB")

            if not is_tissue_patch(patch):
                continue

            x_tensor = transform(patch).unsqueeze(0).to(cfg.DEVICE)
            feat = uni_model(x_tensor)
            ...
            logits_patch = patch_model(feat.unsqueeze(0).to(cfg.DEVICE))
            score_patch = torch.softmax(logits_patch, dim=1).max().item()
            patch_scores.append(score_patch)

    slide.close()

    if len(embeddings) == 0:
        print(f"No tissue patches found for heatmap: {slide_path}")
        return

    bag_x = torch.stack(embeddings).to(cfg.DEVICE)

    with torch.no_grad():
        logits, att = mil_model(bag_x, return_attention=True)
        probs = torch.softmax(logits, dim=0).cpu().numpy()
        pred_class = int(np.argmax(probs))
        att = att.squeeze(1).detach().cpu().numpy()

    att_norm = (att - att.min()) / (att.max() - att.min() + 1e-8)

    heat = np.zeros((len(ys), len(xs)), dtype=np.float32)
    patch_heat = np.zeros((len(ys), len(xs)), dtype=np.float32)

    x_to_ix = {x: i for i, x in enumerate(xs)}
    y_to_iy = {y: i for i, y in enumerate(ys)}

    for (x, y), val in zip(valid_coords, att_norm):
        heat[y_to_iy[y], x_to_ix[x]] = val

    for (x, y), val in zip(valid_coords, patch_scores):
        patch_heat[y_to_iy[y], x_to_ix[x]] = val

    slide = openslide.OpenSlide(slide_path)
    thumb_max = 2048
    scale = max(w, h) / thumb_max if max(w, h) > thumb_max else 1.0
    thumb_w = max(1, int(w / scale))
    thumb_h = max(1, int(h / scale))
    thumb = slide.get_thumbnail((thumb_w, thumb_h)).convert("RGB")
    slide.close()

    heat_img = Image.fromarray((heat * 255).astype(np.uint8)).resize((thumb_w, thumb_h), resample=Image.BILINEAR)
    heat_resized = np.array(heat_img).astype(np.float32) / 255.0

    title = f"MIL Attention Heatmap - {slide_name} | Pred: {class_names[pred_class]}"
    ...
    plt.title(title)
    plt.tight_layout()
    plt.savefig(os.path.join(cfg.HEATMAP_DIR, f"{save_prefix}_{slide_name}_mil_attention.png"), dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.axis("off")
    plt.tight_layout()
    plt.savefig(os.path.join(cfg.HEATMAP_DIR, f"{save_prefix}_{slide_name}_mil_overlay.png"), dpi=300, bbox_inches="tight")
    plt.close()

    patch_heat_img = Image.fromarray((patch_heat * 255).astype(np.uint8)).resize((thumb_w, thumb_h), resample=Image.BILINEAR)
    patch_heat_resized = np.array(patch_heat_img).astype(np.float32) / 255.0

    plt.figure(figsize=(12, 10))
    plt.imshow(thumb)
    plt.imshow(patch_heat_resized, cmap="magma", alpha=0.45)
    plt.title(f"Patch Score Overlay - {slide_name}")
    plt.axis("off")
    plt.tight_layout()
    plt.savefig(os.path.join(cfg.HEATMAP_DIR, f"{save_prefix}_{slide_name}_patch_overlay.png"), dpi=300, bbox_inches="tight")
    plt.close()
    ...
    heatmap_info = {
        ...
        "n_valid_heatmap_patches": len(valid_coords),
        "stride": cfg.HEATMAP_STRIDE,
        "patch_size": cfg.PATCH_SIZE,
    }
    save_json(heatmap_info, os.path.join(cfg.HEATMAP_DIR, f"{save_prefix}_{slide_name}_summary.json"))
...
def main():
    ...
    if cfg.GENERATE_HEATMAPS:
        print("\nGenerating attention heatmaps for the first test slides...\n")
        n_heatmaps = min(3, len(test_s))

        for i in range(n_heatmaps):
            try:
                create_attention_heatmap(
                    slide_path=test_s[i]["path"],
                    uni_model=uni,
                    mil_model=mil_model,
                    ...
                    transform=transform,
                    cfg=cfg,
                    save_prefix=f"test_{i + 1}",
                )
            except Exception as e:
                print(f"Failed to generate heatmap for {test_s[i]['path']}: {e}")
